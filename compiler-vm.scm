...
(define-macro (import-only module . syms) `(begin ,@(map (lambda (sym) `(define ,sym (with-module ,module ,sym))) syms)))(import-only gauche.internal extended-pair? extended-cons extended-list pair-attribute-get pair-attribute-set! pair-attributes)(define *command-line-args* '())(define (command-line) *command-line-args*)(define make-eq-hashtable make-hash-table)(define hashtable-set! hash-table-put!)(define hashtable-ref hash-table-get)(define dd (lambda a '()))(define pp (lambda a '()))(define syntax-error error)(define find10 find)(define append2 append)(define (source-info p) (let1 src (debug-source-info p) (if (pair? src) (cons (sys-basename (car src)) (cdr src)) src)))(define (make-list-with-src-slot lst) (apply extended-list lst))(define (set-source-info! a b) (cond ((extended-pair? a) (pair-attribute-set! a 'source-info b) a) (else a)))(define append! append)(define-macro
  (first o)
...
(define
  ...
  (define
    ...
    (let ((then (if (> (length then) 1)
                    (quasiquote (begin (unquote-splicing then)))
                    (car then))))
         (quasiquote
       ...
       (if (null? clauses)
           ...
           (cond ((and (null? (cdr clauses))
                  ...
                  (if (> (length (cdar clauses)) 1)
                      (quasiquote
                        (begin (unquote-splicing (cdar clauses))))
                      (cadar clauses)))
  ...
  (let* ((vars (second sexp))
        ...
        ($lambda
          (cons (source-info sexp)
                (quasiquote
                  ((unquote name)
                   (unquote-splicing (dotpair->list (second sexp))))))
          name
          (if optional-arg?
              (- (length vars) 1)
              (length vars))
                       ...
                       (((quote define) name ((quote lambda) . more))
                        (let1 closure
                              (make-list-with-src-slot
                                (quasiquote (lambda (unquote-splicing more))))
                              (set-source-info!
                                closure
                ...
                ((= $LET t)
                 (append
                   ($append-map1
                     ...
                     ($let.inits i))
                   (rec ($let.body i)
                        (append l ($let.lvars i))
                        labels-seen)))
                ((= $RECEIVE t)
                 (append
                   (rec ($receive.vals i) l labels-seen)
                   (rec ($receive.body i)
                        (append l ($receive.lvars i))
                        labels-seen)))
                ((= $SEQ t)
                 ($append-map1
                   (lambda (fm) (rec fm l labels-seen))
         ...
         (let* ((label ($lambda.body ($call.proc iform)))
                ...
                (args-code
                  (pass3/compile-args
                    ...
                    #f))
                (sets-here
                  (append (pass3/find-sets body vars) sets))
